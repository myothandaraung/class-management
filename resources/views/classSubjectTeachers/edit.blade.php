    ...
    <div class="row">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center mb-5">
                <h1 class="text-primary fw-bold mb-0">関連の編集</h1>
            ...
            <div class="card shadow-lg border-0 rounded-4">
                <div class="card-body p-5">
                    <form action="{{ route('classSubjectTeachers.update', $classSubjectTeacher) }}" method="POST">
                        @csrf
                        @method('PUT')
                                ...
                                <select name="class_id" id="class_id" class="form-select @error('class_id') is-invalid @enderror">
                                    ...
                                    @foreach($classes as $class)
                                        <option value="{{ $class->id }}" 
                                                {{ $class->id == $classSubjectTeacher->class_id ? 'selected' : '' }}>
                                ...
                                <select name="subject_id" id="subject_id" class="form-select @error('subject_id') is-invalid @enderror">
                                    ...
                                    @foreach($subjects as $subject)
                                        <option value="{{ $subject->id }}"
                                                {{ $subject->id == $classSubjectTeacher->subject_id ? 'selected' : '' }}>
                                ...
                                <select name="teacher_id" id="teacher_id" class="form-select @error('teacher_id') is-invalid @enderror">
                                    ...
                                    @foreach($teachers as $teacher)
                                        <option value="{{ $teacher->id }}"
                                                {{ $teacher->id == $classSubjectTeacher->teacher_id ? 'selected' : '' }}>
