    ...
    <div class="row">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h2 class="text-primary fw-bold">科目の編集</h2>
                <div class="btn-group">
                    <a href="{{ route('subjects.show', $subject) }}" class="btn btn-sm btn-outline-primary me-2">
            ...
            <div class="card shadow-sm border-0">
                <div class="card-body p-4">
                    <form action="{{ route('subjects.update', $subject) }}" method="POST" class="row g-4">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <select class="form-select border-0 shadow-sm @error('type') is-invalid @enderror" 
                                    ...
                                    <option value="必修" {{ old('type', $subject->type) == '必修' ? 'selected' : '' }}>必修</option>
                                    <option value="選択" {{ old('type', $subject->type) == '選択' ? 'selected' : '' }}>選択</option>
                                    <option value="特別" {{ old('type', $subject->type) == '特別' ? 'selected' : '' }}>特別</option>
                                </select>
                            </div>
                            @error('type')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                                ...
                                <select class="form-select border-0 shadow-sm @error('courses') is-invalid @enderror" 
                                    ...
                                    @foreach($courses as $course)
                                        <option value="{{ $course->id }}" 
                                                {{ in_array($course->id, old('courses', $subject->courses->pluck('id')->toArray())) ? 'selected' : '' }}>
                                            {{ $course->name }} ({{ $course->year }}年度)
                                        </option>
                                    @endforeach
                                </select>
                            </div>
